...
func TestNextToken(t *testing.T) {
	...
	for i, testCase := range testCases {
		tok := lexer.NextToken()
		if tok.Type != testCase.expectedToken {
			t.Fatalf("tests[%d] - tokentype wrong. expected=%q, got=%q", i, testCase.expectedToken, tok.Type)
		}
...
func TestNextTokenExtendFurther(t *testing.T) {
	/*
		Note that although the input looks like an actual piece of Monkey source code, some lines don’t
		really make sense, with gibberish like !-/*5.

		That’s okay. The lexer’s job is not to tell us whether code makes sense, works or contains errors. That comes in a later stage.
