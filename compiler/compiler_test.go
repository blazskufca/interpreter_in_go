...
type compilerTestCase struct {
	input                string
	expectedConstants    []any // expectedConstants are the constants we expect to see in the constants pool
...
func TestIntegerArithmetic(t *testing.T) {
	tests := []compilerTestCase{
		...
		// Test that the compiler emits code.OpPop after every expression statement, i.e. don't save value's on the stack which shouldn't be there anymore
		{
			input:             `1; 2;`,
			expectedConstants: []any{1, 2},
			expectedInstructions: []code.Instructions{
...
func TestBooleanExpressions(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             "1 < 2",
			expectedConstants: []any{2, 1}, // Look at the operands here and in all the other cases...They are reversed because we don't have a OpLessThan, instead bytecode itself is 'reversed'!
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpGreaterThan),
	...
	tests := []compilerTestCase{
		{
			...
			expectedConstants: []any{
				5,
				10,
				[]code.Instructions{ // This is the compiled function in the constants pool. This is test is testing specifically this for now!
					code.Make(code.OpConstant, 0),
					code.Make(code.OpConstant, 1),
					code.Make(code.OpAdd),
		...
		// Test that the last code.OpPop was replaced with code.OpReturnValue, which it has to be (i.e. implicit return)!
		{
			input: `fn() { 1; 2 }`,
			expectedConstants: []any{
				1,
...
func TestLetStatementScopes(t *testing.T) {
	tests := []compilerTestCase{
		...
		// Test that functions create local variables not global ones
		{
		...
		// Test that we can create a multiple local variables
		{
